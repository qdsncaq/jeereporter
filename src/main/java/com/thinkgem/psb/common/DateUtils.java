...
public class DateUtils extends Object {
    ...
    public static Date getMonthLastDay(Date date) {
        ...
        String certainDate = getCertainDate(nextMonthFirstDay, -1);
        Date result = null;
        result = getDateFromString(certainDate, "yyyy-MM-dd");
        return result;
    }
    ...
    //昨天
    public static Date lastDay(Date date) throws Exception {
    ...
    //上周日
    public static Date currMonday(Date date)throws Exception{
        
        Calendar cal = Calendar.getInstance();
        cal.setTime(DateUtils.lastMonday(date));
        cal.set(Calendar.HOUR_OF_DAY, 0);
        cal.set(Calendar.SECOND,0);
        cal.set(Calendar.MINUTE,0);
    ...
    /**取两个日期之间的天数() date2-date1*/
    public static BigDecimal getDays(Date date1,Date date2) throws Exception {
		int days = 0;
